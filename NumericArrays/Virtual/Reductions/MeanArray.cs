namespace NumericArrays.Virtual.Reductions {
    ...
    public class MeanArray<T> : NumericArray<T>, IReductionArray<T>
        ...
        public MeanArray(INumericArray sourceArray, int? axis) :
        ...
        public override T this[int linearIndex] {
            ...
            get {
                ...
                double valueSum = _sourceArray[start];

                for (int i = start + _axisStep; i < end; i += _axisStep)
                {
                    valueSum += _sourceArray[i];
                }

                return (T)(valueSum/_axisIterations).CastToArrayValueType(ValueType);
            }
        ...
        public override INumericArray<T> Clone() => new MeanArray<T>(_sourceArray, _axis);
        #endregion
